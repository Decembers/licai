...
class Commodity extends Controller
{
    ...
    public function comadd()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $data['com_number'] = 'YAN'.date("Ymd").rand(1000,9999);//订单编号
            ...
            if ($data['return_mode'] == 1) {
                ...
                $data['nexpect'] = 1;//按期返还
                $data['expect'] = substr(sprintf("%.5f",$data['return_price']/100 * $data['price'] / 12 * $yer),0,-1); //按期返还利润
            }
    ...
    {
        $list = C::where(['automation'=>['neq',0]])->select();
    ...
    public function auedit()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $data['content'] = $content;
            $data['numbers'] = $data['number'];
            $data['down_time'] = $data['down_time']*3600;
            $data['deal_time'] = $data['deal_time']*86400;
            $data['convert_time']=86400 * $data['convert_time'];
            ...
            $vo['down_time'] = $vo['down_time']/3600;
            $vo['deal_time'] = $vo['deal_time']/86400;
            $vo['convert_time'] = $vo['convert_time']/86400;
