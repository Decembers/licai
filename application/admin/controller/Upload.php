...
class Upload extends Controller
{
    ...
    public function upload()
    {
        ...
        $path = ROOT_PATH . 'public/tmp/uploads/management/';
        $info = $file->move($path);
        if (!$info) {
            return ajax_return_error($file->getError());
        }
        $data = '/tmp/uploads/management/' . $info->getSaveName();
    ...
    public function remote()
    {
        ...
        // validate
        $name = ROOT_PATH . 'uploads/management/' . get_random();
        $name = \File::downloadImage($url, $name);

        $ret = $this->request->root() . 'uploads/management/' . basename($name);
